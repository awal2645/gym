...
export default function Chat() {
    ...
    useEffect(() => {
        ...
        if (pusher && user?.id) {
            try {
                // Subscribe to private channel (Pusher will automatically authenticate)
                const channel = pusher.subscribe(`private-chat.user.${user.id}`);
                ...
                channel.bind('message.sent', (data) => {
                    ...
                    // Only notify when message is from someone else (e.g. admin/Joey)
                ...
                return () => {
                    if (channelRef.current) {
                        ...
                        pusher.unsubscribe(`private-chat.user.${user.id}`);
                    }
                };
            } catch (error) {
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            if (fileToUpload && fileToUpload.size > CHUNK_THRESHOLD) {
                // Large file: chunked upload (smooth, batch-wise, up to ~1GB)
                const chunkedUrl = `${baseUrl}/chat/upload`;
    ...
    return (
        <div className="min-h-screen bg-black text-white flex flex-col">
            <Navbar />
            <Sidebar />
            <main className="flex-1 flex flex-col min-h-0 lg:ml-64 pt-20 sm:pt-24">
                <div className="flex-1 flex flex-col min-h-0 p-4 sm:p-6 lg:p-8 pb-6 sm:pb-8">
                    <div className="flex-1 flex flex-col min-h-0 max-w-4xl mx-auto w-full">
                        <h3 className="text-xl sm:text-3xl lg:text-4xl font-bold mb-4 sm:mb-6 lg:mb-8 shrink-0 bg-gradient-to-r from-white to-white/70 bg-clip-text text-transparent">Chat with Joey - Upload Your Video & Message</h3>

                        {successMessage && (
                            <div className="shrink-0 bg-green-500/10 border border-green-500/50 text-green-400 px-4 py-3 rounded-xl mb-4">
                                {successMessage}
                            </div>
                        )}
                        ...
                        <Card className="flex flex-col flex-1 min-h-0 p-0 overflow-hidden">
                            ...
                            {uploading && (selectedFile?.type?.startsWith('video/') || ['mp4', 'webm', 'mkv', 'mov', 'avi', 'm4v', 'ogv', '3gp'].includes((selectedFile?.name || '').split('.').pop()?.toLowerCase())) && (
                                <div className="shrink-0 mt-4 mx-6 mb-6 p-4 rounded-xl bg-blue-500/10 border border-blue-500/30">
                                    ...
                                    <p className="text-white/70 text-sm mt-1 italic">Longer videos may take up to a few minutes. Please do not close this browser.</p>
                                </div>
                            )}
                        </Card>
                    </div>
